...
func (c *Context) Get(node int) []{{.Type}} {
	if c.Cache != nil {
		return c.Cache[node]
	}
	return nil
...
func (c *Context) Set(node int, value []{{.Type}}) {
	if c.Cache != nil {
		c.Cache[node] = value
	}
}
...
func (context *Context) Mul(k Continuation, node int, a, b *V) bool {
	...
	if a.Seed != 0 {
		...
		go func() {
			...
			derivatives := func(index int, ad []{{.Type}}) {
				rows, bi := a.S[1], 0
				for i := 0; i < sizeB; i += width {
...
func (context *Context) Everett(k Continuation, node int, a *V) bool {
	...
	if a.Seed != 0 {
		c.Seed, c.Drop = a.Seed, a.Drop
		index, dropout, factor := 0, uint32((1 - a.Drop) * math.MaxUint32), {{.Type}}(1/(1 - a.Drop))
		if cached == nil {
			for i := 0; i < a.S[1]; i++ {
				rng := a.Seed
				for j := 0; j < a.S[0]; j++ {
...
func (context *Context) Complex(k Continuation, node int, a, b *V) bool {
	if len(a.S) != 2 || len(b.S) != 2 {
		panic("tensor needs to have two dimensions")
	}
	width, length := a.S[0], len(b.X)
