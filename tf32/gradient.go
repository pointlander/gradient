...
func (c *Context) Get(node int) []float32 {
	if c.Cache != nil {
		return c.Cache[node]
	}
	return nil
...
func (c *Context) Set(node int, value []float32) {
	if c.Cache != nil {
		c.Cache[node] = value
	}
}
...
func (context *Context) Everett(k Continuation, node int, a *V, options ...map[string]interface{}) bool {
	...
	if a.Seed != 0 {
		c.Seed, c.Drop = a.Seed, a.Drop
		index, dropout, factor := 0, uint32((1-a.Drop)*math.MaxUint32), float32(1/(1-a.Drop))
		if cached == nil {
			for i := 0; i < a.S[1]; i++ {
				rng := a.Seed
				for j := 0; j < a.S[0]; j++ {
...
func (context *Context) Variance(k Continuation, node int, a *V, options ...map[string]interface{}) bool {
	...
	length := a.S[1]
	c, size, width, means := NewV(length), len(a.X), a.S[0], make([]float32, 0, length)
